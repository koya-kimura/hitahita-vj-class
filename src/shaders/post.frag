...
void main(void) {
    ...
    // fx0: モザイク
    float mosaicGrid = mix(128.0, 8.0, clamp(u_fx[0], 0.0, 1.0));
    vec2 mosaicUv = (floor(suv * mosaicGrid) + 0.5) / mosaicGrid;
    suv = mix(suv, mosaicUv, u_fx[0]);
    ...
    // fx5: 左右ミラーリング（左半分を右半分へミラー）
    float mirroredX = (suv.x <= 0.5) ? suv.x : (1.0 - suv.x);
    vec2 mirrorUv = vec2(mirroredX, suv.y);
    suv = mix(suv, mirrorUv, fx5On);

    // fx6: グリッチ (少し大きめ)
    float line = step(0.825, fract((uv.y + u_time * 0.22) * 360.0));
    float jitter = (rand2(vec2(floor(uv.y * 360.0), floor(u_time * 28.0))) - 0.5) * 0.18;
